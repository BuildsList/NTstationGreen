...
/datum/wires/syndicatebomb/UpdatePulsed(var/index)
	...
	switch(index)
		if(WIRE_BOOM)
			if (P.active)
				P.loc.visible_message("<span class='info'>\icon[holder] An alarm sounds! It's go-</span>")
				P.timer = 0
		if(WIRE_UNBOLT)
			P.loc.visible_message("<span class='info'>\icon[holder] The bolts spin in place for a moment.</span>")
		if(WIRE_DELAY)
			playsound(P.loc, 'sound/machines/chime.ogg', 30, 1)
			P.loc.visible_message("<span class='info'>\icon[holder] The bomb chirps.</span>")
			P.timer += 10
		if(WIRE_PROCEED)
			playsound(P.loc, 'sound/machines/buzz-sigh.ogg', 30, 1)
			P.loc.visible_message("<span class='info'>\icon[holder] The bomb buzzes ominously!</span>")
		...
		if(WIRE_ACTIVATE)
			if(!P.active && !P.defused)
				...
				P.loc.visible_message("<span class='info'>\icon[holder] You hear the bomb start ticking!</span>")
				P.active = 1
				P.icon_state = "[initial(P.icon_state)]-active[P.open_panel ? "-wires" : ""]"
			else
				P.loc.visible_message("<span class='info'>\icon[holder] The bomb seems to hesitate for a moment.</span>")
				P.timer += 5
	...
	switch(index)
		...
		if(WIRE_UNBOLT)
			if (!mended && P.anchored)
				...
				P.loc.visible_message("<span class='info'>\icon[holder] The bolts lift out of the ground!</span>")
				P.anchored = 0
		if(WIRE_PROCEED)
			if(!mended && P.active)
				P.loc.visible_message("<span class='info'>\icon[holder] An alarm sounds! It's go-</span>")
		...
		if(WIRE_ACTIVATE)
			if (!mended && P.active)
				P.loc.visible_message("<span class='info'>\icon[holder] The timer stops! The bomb has been defused!</span>")

...
/obj/effect/spider/stickyweb/CanPass(atom/movable/mover, turf/target, height=0, air_group=0)
	...
	if(istype(mover, /mob/living/simple_animal/hostile/giant_spider))
		...
		if(prob(50))
			mover << "<span class='info'>You get stuck in \the [src] for a moment.</span>"
			return 0
...
/obj/effect/spider/spiderling/process()
			...
			if(entry_vent.network && entry_vent.network.normal_members.len)
				...
				spawn(rand(20,60))
					...
					spawn(travel_time)
						...
						if(prob(50))
							src.visible_message("<span class='info'>You hear something squeezing through the ventilation ducts.</span>",2)
						sleep(travel_time)
	...
	//=================
		...
		if(nearby.len)
			...
			if(prob(40))
				src.visible_message("<span class='info'>\The [src] skitters[pick(" away"," around","")].</span>")
	else if(prob(10))
		//ventcrawl!
...
/obj/effect/spider/cocoon/Destroy()
	src.visible_message("<span class='info'>\The [src] splits open.</span>")

...
/obj/structure/sign/garland
	name = "garland"
	desc = "A strange rope with lights, that gleams."
	icon = 'icons/obj/christmas.dmi'
	icon_state = "garland_s"
	luminosity = 1
	anchored = 1

/obj/structure/sign/garland/attack_hand(mob/user)
	var/temp_loc = user.loc
	switch(alert("Do I want to rip the garland from the wall?","You think...","Yes","No"))
		...
		if("No")
			return
/*
/obj/structure/sign/garland/attackby(obj/item/I, mob/user)
	if(istype(I, /obj/item/stack/coil/garland))
		...
		return
*/
/obj/structure/sign/garland/proc/remove_garland(turf/location)
	var/obj/structure/sign/garland/P = new(src)
	P.loc = location
	loc = P
...
/turf/simulated/wall/proc/place_garland(obj/item/stack/coil/garland/P, mob/user)
	...
	for(var/obj/O in contents) //Let's see if it already has a garland on it.
		if(istype(O,/obj/structure/sign/garland))
	...
	var/obj/structure/sign/garland/D = P.resulting_garland

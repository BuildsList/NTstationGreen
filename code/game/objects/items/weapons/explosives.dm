...
/obj/item/weapon/plastique/afterattack(atom/movable/target, mob/user, flag)
	...
	if(ismob(target))
		...
		user.visible_message("<span class='info'>[user.name] is trying to plant some kind of explosive on [target.name]!</span>")
	...
	if(do_after(user, 50) && in_range(user, target))
		...
		if (ismob(target))
			...
			user.visible_message("<span class='info'>[user.name] finished planting an explosive on [target.name]!</span>")

...
/obj/item/weapon/storage/lockbox/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if (W.GetID())
		if(src.broken)
			user << "<span class='info'>It appears to be broken.</span>"
			return
		if(src.allowed(user))
			src.locked = !( src.locked )
			if(src.locked)
				src.icon_state = src.icon_locked
				user << "<span class='info'>You lock the [src.name]!</span>"
				return
			else
				src.icon_state = src.icon_closed
				user << "<span class='info'>You unlock the [src.name]!</span>"
				return
		else
		...
		if(istype(W, /obj/item/weapon/melee/energy/blade))
			...
			for(var/mob/O in viewers(user, 3))
				O.show_message(text("<span class='info'>\The [src] has been sliced open by [] with an energy blade!</span>", user), 1, text("<span class='info'>You hear metal being sliced and sparks flying.</span>"), 2)
			return
		else
			for(var/mob/O in viewers(user, 3))
	...
	if(!locked)
		...
		user << "<span class='info'>It's locked!</span>"
	return

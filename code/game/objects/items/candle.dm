...
/obj/item/candle
	...
	var/lit = 0
	proc
		light(var/flavor_text = "<span class='info'>[usr] lights the [name].</span>")
...
/obj/item/candle/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if(istype(W, /obj/item/weapon/weldingtool))
		...
		if(WT.isOn()) //Badasses dont get blinded by lighting their candle with a welding tool
			light("<span class='info'>[user] casually lights the [name] with [W], what a badass.</span>")
...
/obj/item/candle/light(var/flavor_text = "<span class='info'>[usr] lights the [name].</span>")
	if(!src.lit)
		src.lit = 1
		//src.damtype = "fire"

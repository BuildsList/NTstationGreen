...
/obj/item/device/camera_bug/check_eye(var/mob/user as mob)
	...
	if(T.z != current.z || (!skip_bugcheck && current.bug != src) || !current.can_use())
		user << "<span class='info'>[src] has lost the signal.</span>"
		current = null
		user.reset_view(null)
		user.unset_machine()
		return null
...
/obj/item/device/camera_bug/Topic(var/href,var/list/href_list)
	...
	if("view" in href_list)
		...
		if(istype(C))
			if(!C.can_use())
				usr << "<span class='info'>Something's wrong with that camera.  You can't get a feed.</span>"
				return
			var/turf/T = get_turf(loc)
			if(!T || C.z != T.z)
				usr << "<span class='info'>You can't get a signal.</span>"
				return
			current = C
			spawn(6)

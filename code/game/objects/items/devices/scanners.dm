...
/obj/item/device/healthanalyzer/attack(mob/living/M as mob, mob/living/user as mob)
	...
	if (user.has_organic_effect(/datum/organic_effect/clumsy) || (user.getBrainLoss() >= 60) && prob(50))
...
/proc/healthscan(var/mob/living/user, var/mob/living/M, var/mode = 1)
	...
	user.show_message(text("<span class='notice'>[] | [] | [] | []</span>", oxy_loss > 50 ? "<span class='info'>Severe oxygen deprivation detected\blue</span>" : "Subject bloodstream oxygen level normal", tox_loss > 50 ? "<span class='info'>Dangerous amount of toxins detected\blue</span>" : "Subject bloodstream toxin level minimal", fire_loss > 50 ? "<span class='info'>Severe burn damage detected\blue</span>" : "Subject burn injury status O.K", brute_loss > 50 ? "<span class='info'>Severe anatomical damage detected\blue</span>" : "Subject brute-force injury status O.K"), 1)
...
/obj/item/device/analyzer/attack_self(mob/user as mob)
	...
	if(total_moles)
		...
		if(unknown_concentration > 0.01)
			user.show_message("<span class='info'>Unknown: [round(unknown_concentration*100)]%</span>", 1)
...
/obj/item/device/mass_spectrometer/attack_self(mob/user as mob)
	if (user.stat)
		return
	if (crit_fail)
		user << "<span class='info'>This device has critically failed and is no longer functional!</span>"
		return
	...
	if(reagents.total_volume)
		...
		for(var/datum/reagent/R in reagents.reagent_list)
			if(R.id != "blood")
				...
				user << "<span class='info'>The sample was contaminated! Please insert another sample</span>"
				return
			else

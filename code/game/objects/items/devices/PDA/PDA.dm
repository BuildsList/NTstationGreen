...
//MESSENGER/NOTE FUNCTIONS===================================
			...
			if ("Edit")
				...
				if (in_range(src, U) && loc == U)
					n = sanitize(copytext(adminscrub(n), 1, MAX_MESSAGE_LEN))
					if (mode == 1)
						note = replacetext(n, "\n", "<BR>")
						notehtml = n
				else
				...
				if(istype(cartridge, /obj/item/weapon/cartridge/clown))//Cartridge checks are kind of unnecessary since everything is done through switch.
					...
					if(!isnull(P))
						if (!P.toff && cartridge:honk_charges > 0)
							...
							U.show_message("<span class='info'>Virus sent!</span>", 1)
					...
					if(!isnull(P))
						if (!P.toff && cartridge:shock_charges > 0)
							...
							if(prob(difficulty * 12) || (P.hidden_uplink))
								U.show_message("<span class='info'>An error flashes on your [src].</span>", 1)
							else if (prob(difficulty * 3))
								U.show_message("<span class='info'>Energy feeds back into your [src]!</span>", 1)
								U << browse(null, "window=pda")
								explode()
							else
								U.show_message("<span class='info'>Success!</span>", 1)
								P.explode()
					else
						U << "PDA not found."
				else
		...
		switch(scanmode)
			...
			if(4)
				for (var/mob/O in viewers(C, null))
					O.show_message("<span class='info'>[user] has analyzed [C]'s radiation levels!</span>", 1)

				user.show_message("<span class='info'>Analyzing Results for [C]:</span>")
				if(C.radiation)
					user.show_message("\green Radiation Level: \black [C.radiation]")
				else
					user.show_message("<span class='info'>No radiation detected.</span>")
...
/obj/item/device/pda/afterattack(atom/A as mob|obj|turf|area, mob/user as mob, proximity)
		...
		if(3)
			if(!isnull(A.reagents))
				if(A.reagents.reagent_list.len > 0)
					...
					user << "<span class='info'>[reagents_length] chemical agent[reagents_length > 1 ? "s" : ""] found.</span>"
					for (var/re in A.reagents.reagent_list)
						user << "<span class='info'>\t [re]</span>"
				else
					user << "<span class='info'>No active chemical agents found in [A].</span>"
			else
				user << "<span class='info'>No significant chemical agents found in [A].</span>"
	...
	if (!scanmode && istype(A, /obj/item/weapon/paper) && owner)
		note = A:info
		user << "<span class='info'>Paper scanned.</span>" //concept of scanning paper copyright brainoblivion 2009
...
/obj/item/device/pda/proc/explode() //This needs tuning.
	...
	if (ismob(loc))
		var/mob/M = loc
		M.show_message("<span class='info'>Your [src] explodes!</span>", 1)

/client/verb/ooc(msg as text)
	...
	if(say_disabled)	//This is here to try to identify lag problems
		usr << "<span class='info'>Speech is currently admin-disabled.</span>"
		return
	...
	if(!(prefs.toggles & CHAT_OOC))
		src << "<span class='info'>You have OOC muted.</span>"
		return

	if(!holder)
		if(!ooc_allowed)
			src << "<span class='info'>OOC is globally muted</span>"
			return
		if(!dooc_allowed && (mob.stat == DEAD))
			usr << "<span class='info'>OOC for dead mobs has been turned off.</span>"
			return
		if(prefs.muted & MUTE_OOC)
			src << "<span class='info'>You cannot use OOC (muted).</span>"
			return

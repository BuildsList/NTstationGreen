...
/obj/machinery/mecha_part_fabricator/proc/output_parts_list(set_name)
	...
	if(istype(part_set))
		world.log << "([time_stamp()]) Mechafab (\ref[src])(output_parts_list) output set_name - [set_name] ; part_set - \ref[part_set]"
...
/obj/machinery/mecha_part_fabricator/proc/process_queue()
	...
	while(part)
		...
		if(!check_resources(part))
			src.visible_message("\icon[src] <b>\The [src]</b> 	, \"Not enough resources. Queue processing stopped.\"")
...
/obj/machinery/mecha_part_fabricator/attackby(obj/W as obj, mob/user as mob)
	...
	if(panel_open)
		if(istype(W, /obj/item/weapon/crowbar))
			...
			user << "<span class='warning'>You can't load \the [src.name] while it's opened.</span>"
			return 1

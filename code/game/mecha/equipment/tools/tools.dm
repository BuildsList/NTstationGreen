...
/obj/item/mecha_parts/mecha_equipment/tool/drill
	...
	action(atom/target)
		...
		var/mob/living/carbon/mecha_occupant = chassis.occupant
		if(do_after_cooldown(target))
			if(T == chassis.loc && src == chassis.selected)
				if(istype(target, /turf/simulated/wall/r_wall))
					...
					if(ismob(target))
						var/mob/M = target
						add_logs(chassis.occupant, M, "attacked", object="[name]", addition="(INTENT: [uppertext(mecha_occupant.a_intent)]) (DAMTYE: [uppertext(damtype)])")
					target.ex_act(2)
		return 1

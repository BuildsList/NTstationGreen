...
//This proc is called before the level loop of DivideOccupations() and will try to select a head, ignoring ALL non-head preferences for every level untilґ
...
/datum/controller/occupations/proc/EquipRank(var/mob/living/H, var/rank, var/joined_late = 0)
	...
	H << "<b>Как [job.r_title], вы подчин&#255;етесь непосредственно [job.r_supervisors]. Это может изменитьс&#255; при особых обсто&#255;тельствах.</b>"
	H << "<b>Чтобы разговаривать по радиоканалу своего отдела используй приписку :h. Рассмотрите свой наушник, чтобы узнать доступные радиоканалы.</b>"

...
/obj/item/weapon/veilrender/attack_self(mob/user as mob)
	if(charged == 1)
		...
		charged = 0
		visible_message("<span class='info'><B>[src] hums with power as [usr] deals a blow to reality itself!</B></span>")
	else
		user << "<span class='info'>The unearthly energies that powered the blade are now dormant.</span>"
...
/obj/item/weapon/veilrender/vealrender/attack_self(mob/user as mob)
	if(charged)
		...
		charged = 0
		visible_message("<span class='info'><B>[src] hums with power as [usr] deals a blow to hunger itself!</B></span>")
	else
		user << "<span class='info'>The unearthly energies that powered the blade are now dormant.</span>"
...
/obj/effect/rend/cow/attackby(obj/item/I as obj, mob/user as mob)
	if(istype(I, /obj/item/weapon/nullrod))
		visible_message("<span class='info'><b>[I] strikes a blow against \the [src], banishing it!</b></span>")
		qdel(src)
		return
	..()
...
/obj/item/weapon/scrying/attack_self(mob/user as mob)
	...
	visible_message("<span class='info'><B>[usr] stares into [src], their eyes glazing over.</B></span>")
	user.ghostize(1)
	return
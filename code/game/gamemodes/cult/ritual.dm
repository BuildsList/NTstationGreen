...
// join hide technology - stun rune. Rune color: bright pink.
	...
	attack_hand(mob/living/user as mob)		// OH GOD this is horrible
		...
		if(word1 == wordtravel && word2 == wordhell && word3 == wordtech)
			...
			user << "<span class='info'>You feel Lord Nar-Sie is displeased with you.</span>"
			return fizzle()
	...
	proc
		fizzle()
			...
			for (var/mob/V in viewers(src))
				V.show_message("<span class='info'>The markings pulse with a small burst of light, then fall dark.</span>", 3, "<span class='info'>You hear a faint fizzle.</span>", 2)
			return
	...
	attack(mob/living/M as mob, mob/living/user as mob)
		...
		if(istype(M,/mob/dead))
			...
			user.visible_message( \
				"<span class='info'>[user] drags the ghost to our plane of reality!</span>", \
				"<span class='info'>You drag the ghost to our plane of reality!</span>" \
			)
			return
		if(!istype(M))
			return
		...
		if(iscultist(M))
			if(M.reagents && M.reagents.has_reagent("holywater")) //allows cultists to be rescued from the clutches of ordained religion
				user << "<span class='info'>You remove the taint from [M].</span>"
		...
		for(var/mob/O in viewers(M, null))
			O.show_message(text("<span class='info'><B>[] beats [] with the arcane tome!</B></span>", user, M), 1)
		M << "<span class='info'>You feel searing heat inside!</span>"
	...
	attack_self(mob/living/user as mob)
		...
		if(iscultist(user))
			...
			if (!istype(user.loc,/turf))
				user << "<span class='info'>You do not have enough space to write a proper rune.</span>"
				return
					...
					if("Scribe a rune")		//fixed more assbackward tabbing
						...
						if(usr)
							...
							if (!chosen_rune)
								return
							if (chosen_rune == "none")
								user << "<span class='info'>You decide against scribing a rune, perhaps you should take this time to study your notes.</span>"
								return
						...
						for (var/mob/V in viewers(src))
							V.show_message("<span class='info'>[user] slices open a finger and begins to chant and paint symbols on the floor.</span>", 3, "<span class='info'>You hear chanting.</span>", 2)
						user << "<span class='info'>You slice open one of your fingers and begin drawing a rune on the floor whilst chanting the ritual that binds your life essence with the dark arcane energies flowing through the surrounding world.</span>"
						...
						if(do_after(user, 50))
							...
							user << "<span class='info'>You finish drawing the arcane markings of the Geometer.</span>"

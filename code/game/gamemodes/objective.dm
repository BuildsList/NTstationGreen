...
datum/objective/nuke_steal_crew_member
	...
	explanation_text = "Украсть определённого члена экипажа живым."
...
datum/objective/nuke_steal_crew_member/update_explanation_text()
	if(!target)
		explanation_text = "Свободное задание."
		return
	explanation_text = "Украсть [target.current.name]([target.assigned_job.title]) живым"
	if(should_be_alive)
		explanation_text += "."
	else
		explanation_text += " или мёртвым."
...
datum/objective/steal/exchange/update_explanation_text()
	...
	if(target && target.current)
		...
		explanation_text = "Заполучите [targetinfo.name]."
...
datum/objective/download/proc/gen_amount_goal()
	...
	explanation_text = "Украдите [target_amount] уровней научных исследований."

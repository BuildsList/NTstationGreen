		...
		if (src.yes_code)
			...
			if (href_list["timer"])
				if (src.timing == -1.0)
					return
				if (src.safety)
					usr << "<span class='info'>The safety is still on.</span>"
					return
	...
	if(ticker)
		...
		if(ticker.mode && gamemode_is("nuclear emergency"))
			...
			if(!Nuke.check_finished())//If the mode does not deal with the nuke going off so just reboot because everyone is stuck as is
				...
				kick_clients_in_lobby("<span class='info'>The round came to an end with you in the lobby.</span>", 1) //second parameter ensures only afk clients are kicked
				world.Reboot()
				return
	return

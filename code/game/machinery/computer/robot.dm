...
/obj/machinery/computer/robotics/interact(var/mob/user as mob)
	if (src.z > 6)
		user << "<span class='info'><b>Unable to establish a connection</b>: \black You're too far away from the station!</span>"
		return
	...
	if (href_list["temp"])
			...
			if(can_control(usr, R))
				...
				if(choice == "Confirm" && can_control(usr, R) && !..())
					if(R.mind && R.mind.special_role && R.emagged)
						...
						message_admins("<span class='info'>[key_name_admin(usr)] detonated [R.name]!</span>")
						log_game("<span class='info'>[key_name(usr)] detonated [R.name]!</span>")
						R.self_destruct()
		else
		...
		if(src.allowed(usr))
			...
			if(can_control(usr, R))
				...
				if(choice == "Confirm" && can_control(usr, R) && !..())
					message_admins("<span class='info'>[key_name_admin(usr)] [R.canmove ? "locked down" : "released"] [R.name]!</span>")

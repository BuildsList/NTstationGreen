...
/sound/turntable/test
	file = 'sound/voice/TestLoop1.ogg'
	falloff = 2
	repeat = 1
...
/obj/machinery/party/turntable2/New()
	spawn(rand(3,10))
		var/getmusic = world.Export("http://78.47.53.54/ss13media/listsongs.php")
		if(getmusic)
...
/obj/machinery/party/turntable2/attackby(obj/item/W as obj, mob/user as mob)
...
/obj/machinery/party/turntable2/proc/locktimer()
	if(locktime <= 1)
		src.locked = 0
		locktime = 0
	if(locktime)

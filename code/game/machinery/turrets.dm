...
/obj/machinery/turretid/attackby(obj/item/weapon/W, mob/user)
	...
	if (istype(W, /obj/item/weapon/card/emag) && !emagged)
		user << "<span class='info'>You short out the turret controls' access analysis module.</span>"
		emagged = 1
		locked = 0
		if(user.machine==src)
...
/obj/machinery/turret/attack_animal(mob/living/simple_animal/M as mob)
	...
	if(!(stat & BROKEN))
		visible_message("<span class='info'><B>[M] [M.attacktext] [src]!</B></span>")
...
/obj/machinery/turret/attack_alien(mob/living/carbon/alien/humanoid/M as mob)
	...
	if(!(stat & BROKEN))
		...
		visible_message("<span class='info'><B>[] has slashed at []!</B></span>", M, src)
		src.health -= 15
		if (src.health <= 0)
			src.die()
	else

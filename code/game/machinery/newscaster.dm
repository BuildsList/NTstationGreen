...
/obj/machinery/newscaster/proc/newsAlert(channel)   //This isn't Agouri's work, for it is ugly and vile.
	...
	if(channel)
		for(var/mob/O in hearers(world.view-1, T))
			O.show_message("<span class='newscaster'><EM>[src.name]</EM> ãóäèò, \"Ãëàâíûå íîâîñòè ñ [channel]!\"</span>",2)
		src.alert = 1
		src.update_icon()
		spawn(300)
			src.alert = 0
		...
		for(var/mob/O in hearers(world.view-1, T))
			O.show_message("<span class='newscaster'><EM>[src.name]</EM> ãóäèò, \"Attention! Wanted issue distributed!\"</span>",2)

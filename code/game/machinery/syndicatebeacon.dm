...
/obj/item/device/sbeacondrop/attack_self(mob/user as mob)
	if(user)
		user << "<span class='info'>Locked In</span>"
...
/obj/machinery/singularity_beacon //not the best place for it but it's a hack job anyway -- Urist
	...
	proc/Activate(mob/user = null)
		if(!checkWirePower())
			if(user) user << "<span class='info'>The connected wire doesn't have enough current.</span>"
			return
		...
		active = 1
		if(user) user << "<span class='info'>You activate the beacon.</span>"
	...
	proc/Deactivate(mob/user = null)
		...
		active = 0
		if(user) user << "<span class='info'>You deactivate the beacon.</span>"
	...
	attack_hand(var/mob/user as mob)
		if(stat & SCREWED)
			...
			user << "<span class='info'>You need to screw the beacon to the floor first!</span>"
			return
	...
	attackby(obj/item/weapon/W as obj, mob/user as mob)
		if(istype(W,/obj/item/weapon/screwdriver))
			if(active)
				user << "<span class='info'>You need to deactivate the beacon first!</span>"
				return

			if(stat & SCREWED)
				stat &= ~SCREWED
				anchored = 0
				user << "<span class='info'>You unscrew the beacon from the floor.</span>"
				attached = null
				return
			else
				var/turf/T = loc
				...
				stat |= SCREWED
				anchored = 1
				user << "<span class='info'>You screw the beacon to the floor and attach the cable.</span>"
				return
		..()
		return

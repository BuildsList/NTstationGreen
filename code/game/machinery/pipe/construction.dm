...
/obj/item/pipe/attackby(var/obj/item/weapon/W as obj, var/mob/user as mob)
	...
	for(var/obj/machinery/atmospherics/M in src.loc)
		if(M.initialize_directions & pipe_dir)	// matches at least one direction on either type of pipe
			user << "<span class='info'>There is already a pipe at that location.</span>"
			return 1
	// no conflicts found

	var/pipefailtext = "<span class='info'>There's nothing to connect this pipe section to! (with how the pipe code works, at least one end needs to be connected to something, otherwise the game deletes the segment)</span>"
...
//					world << "[V.node2.name] is connected to valve, forcing it to update its nodes."
	...
	user.visible_message( \
		...
		"<span class='info'>You have fastened the [src].</span>", \
		"You hear ratchet.")
...
/obj/item/pipe_meter/attackby(var/obj/item/weapon/W as obj, var/mob/user as mob)
	...
	if(!locate(/obj/machinery/atmospherics/pipe, src.loc))
		user << "<span class='info'>You need to fasten it to a pipe</span>"
		return 1
	new/obj/machinery/meter( src.loc )
	playsound(src.loc, 'sound/items/Ratchet.ogg', 50, 1)
	user << "<span class='info'>You have fastened the meter to the pipe</span>"
	qdel(src)

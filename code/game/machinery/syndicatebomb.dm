...
/obj/machinery/syndicatebomb/proc/settings(var/mob/user)
	...
	if(in_range(src, user) && isliving(user)) //No running off and setting bombs from across the station
		timer = newtime
		src.loc.visible_message("<span class='info'>\icon[src] timer set for [timer] seconds.</span>")
	if(alert(user,"Would you like to start the countdown now?",,"Yes","No") == "Yes" && in_range(src, user) && isliving(user))
		if(defused || active)
			if(defused)
				src.loc.visible_message("<span class='info'>\icon[src] Device error: User intervention required</span>")
			return
		else
			src.loc.visible_message("<span class='info'>\icon[src] [timer] seconds until detonation, please clear the area.</span>")
...
/obj/item/weapon/bombcore/training/detonate()
	...
	if(istype(holder))
		attempts++
		holder.loc.visible_message("<span class='info'>\icon[holder] Alert: Bomb has detonated. Your score is now [defusals] for [attempts]. Resetting wires...</span>")
		reset()
	else
		qdel(src)
...
/obj/item/weapon/bombcore/training/defuse()
	...
	if(istype(holder))
		attempts++
		defusals++
		holder.loc.visible_message("<span class='info'>\icon[holder] Alert: Bomb has been defused. Your score is now [defusals] for [attempts]! Resetting wires...</span>")
		reset()

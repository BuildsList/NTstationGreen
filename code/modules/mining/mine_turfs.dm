...
/turf/simulated/mineral/random
	...
	icon_state = "rock"
	var/mineralSpawnChanceList = list("Uranium" = 18, "Diamond" = 5, "Gold" = 12, "Silver" = 18, "Plasma" = 25, "Iron" = 40, "Gibtonite" = 2/*, "Adamantine" =5*/, "Cave" = 1)//Currently, Adamantine won't spawn as it has no uses. -Durandan
...
//Watch your tabbing, microwave. --NEO
		if(last_act+W:digspeed > world.time)//prevents message spam
			return
		last_act = world.time
		user << "<span class='info'>You start picking.</span>"
		playsound(user, 'sound/weapons/Genhit.ogg', 20, 1)

		if(do_after(user,W:digspeed))
			user << "<span class='info'>You finish cutting into the rock.</span>"
			gets_drilled()
	else
...
/turf/simulated/floor/plating/asteroid/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if ((istype(W, /obj/item/weapon/shovel)))
		...
		if (dug)
			user << "<span class='info'>This area has already been dug</span>"
			return

		user << "<span class='info'>You start digging.</span>"

...
/obj/item/weapon/coin/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W,/obj/item/stack/cable_coil) )
		...
		if(string_attached)
			user << "<span class='info'>There already is a string attached to this coin.</span>"
			return

		if(CC.amount <= 0)
			user << "<span class='info'>This cable coil appears to be empty.</span>"
			qdel(CC)
			return

		overlays += image('icons/obj/economy.dmi',"coin_string_overlay")
		string_attached = 1
		user << "<span class='info'>You attach a string to the coin.</span>"
		CC.use(1)
		...
		user << "<span class='info'>You detach the string from the coin.</span>"
	else ..()

...
datum/admins/proc/DB_ban_record(var/bantype, var/mob/banned_mob, var/duration = -1, var/reason, var/job = "", var/rounds = 0, var/banckey = null, var/banip = null, var/bancid = null)
	...
	if(blockselfban)
		if(a_ckey == ckey)
			usr << "<span class='info'>You cannot apply this ban type on yourself.</span>"
			return
	...
	if(maxadminbancheck)
		...
		if(adm_query.NextRow())
			...
			if(adm_bans >= MAX_ADMIN_BANS_PER_ADMIN)
				usr << "<span class='info'>You already logged [MAX_ADMIN_BANS_PER_ADMIN] admin ban(s) or more. Do not abuse this function!</span>"
				return

	var/sql = "INSERT INTO erro_ban (`id`,`bantime`,`serverip`,`bantype`,`reason`,`job`,`duration`,`rounds`,`expiration_time`,`ckey`,`computerid`,`ip`,`a_ckey`,`a_computerid`,`a_ip`,`who`,`adminwho`,`edits`,`unbanned`,`unbanned_datetime`,`unbanned_ckey`,`unbanned_computerid`,`unbanned_ip`) VALUES (null, Now(), '[serverip]', '[bantype_str]', '[reason]', '[job]', [(duration)?"[duration]":"0"], [(rounds)?"[rounds]":"0"], Now() + INTERVAL [(duration>0) ? duration : 0] MINUTE, '[ckey]', '[computerid]', '[ip]', '[a_ckey]', '[a_computerid]', '[a_ip]', '[who]', '[adminwho]', '', null, null, null, null, null)"
	var/DBQuery/query_insert = dbcon.NewQuery(sql)
	query_insert.Execute()
	usr << "<span class='info'>Ban saved to database.</span>"
...
datum/admins/proc/DB_ban_unban(var/ckey, var/bantype, var/job = "")
	...
	if(ban_number == 0)
		usr << "<span class='info'>Database update failed due to no bans fitting the search criteria. If this is not a legacy ban you should contact the database admin.</span>"
		return

	if(ban_number > 1)
		usr << "<span class='info'>Database update failed due to multiple bans fitting the search criteria. Note down the ckey, job and current time and contact the database admin.</span>"
		return

	if(istext(ban_id))
		ban_id = text2num(ban_id)
	if(!isnum(ban_id))
		usr << "<span class='info'>Database update failed due to a ban ID mismatch. Contact the database admin.</span>"
		return
...
datum/admins/proc/DB_ban_unban_by_id(var/id)
	...
	if(ban_number == 0)
		usr << "<span class='info'>Database update failed due to a ban id not being present in the database.</span>"
		return

	if(ban_number > 1)
		usr << "<span class='info'>Database update failed due to multiple bans having the same ID. Contact the database admin.</span>"
		return
...
/datum/admins/proc/DB_ban_panel(var/playerckey = null, var/adminckey = null)
	...
	establish_db_connection()
	if(!dbcon.IsConnected())
		usr << "<span class='info'>Failed to establish database connection</span>"
		return

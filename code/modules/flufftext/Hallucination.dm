...
/obj/effect/fake_attacker
	...
	attackby(var/obj/item/weapon/P as obj, mob/user as mob)
		...
		for(var/mob/M in oviewers(world.view,my_target))
			M << "<span class='info'><B>[my_target] flails around wildly.</B></span>"
		my_target.show_message("<span class='info'><B>[src] has been attacked by [my_target] </B></span>", 1) //Lazy.
		...
		if(M == my_target)
			...
			if(prob(30))
				for(var/mob/O in oviewers(world.view , my_target))
					O << "<span class='info'><B>[my_target] stumbles around.</B></span>"
			...
			if(get_dist(src,my_target) > 1)
				...
				if(prob(15))
					if(weapon_name)
						...
						my_target.show_message("<span class='info'><B>[src.name] has punched [my_target]!</B></span>", 1)
						my_target.staminaloss += 30
						if(prob(33))
							if(!locate(/obj/effect/overlay) in my_target.loc)
								fake_blood(my_target)

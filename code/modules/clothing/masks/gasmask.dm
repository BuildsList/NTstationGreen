...
/obj/item/clothing/mask/gas/sechailer/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W, /obj/item/weapon/screwdriver))
		switch(aggressiveness)
			if(1)
				user << "<span class='info'>You set the restrictor to the middle position.</span>"
				aggressiveness = 2
			if(2)
				user << "<span class='info'>You set the restrictor to the last position.</span>"
				aggressiveness = 3
			if(3)
				user << "<span class='info'>You set the restrictor to the first position.</span>"
				aggressiveness = 1
			if(4)
				user << "<span class='info'>You adjust the restrictor but nothing happens, probably because its broken.</span>"
	else if(istype(W, /obj/item/weapon/wirecutters))
		if(aggressiveness != 4)
			user << "<span class='info'>You broke it!</span>"
			aggressiveness = 4
	else
		..()

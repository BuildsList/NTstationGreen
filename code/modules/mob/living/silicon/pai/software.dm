		...
		if("doorjack")
			...
			if(href_list["cable"])
				...
				for (var/mob/M in viewers(T))
					M.show_message("<span class='info'>A port on [src] opens to reveal [src.cable], which promptly falls to the floor.</span>", 3, "<span class='info'>You hear the soft click of something light and hard falling to the ground.</span>", 2)
...
/mob/living/silicon/pai/proc/CheckDNA(mob/living/carbon/M, mob/living/silicon/pai/P)
	...
	if(answer == "Yes")
		...
		for (var/mob/v in viewers(T))
			v.show_message("<span class='info'>[M] presses \his thumb against [P].</span>", 3, "<span class='info'>[P] makes a sharp clicking sound as it extracts DNA material from [M].</span>", 2)

...
//			chargecount = 0
		...
		if(!lockcharge)
			user << "<span class='warning'><b>[src]'s bolts spark! Maybe you should lock them down first!</b></span>"
			spark_system.start()
			return
		else
			playsound(src, 'sound/items/Ratchet.ogg', 50, 1)
			if(do_after(user, 50) && !cell)
				user.visible_message("<span class='warning'>[user] deconstructs [src]!</span>", "<span class='info'>You unfasten the securing bolts, and [src] falls to pieces!</span>")
				deconstruct()
...
//						message_admins("[key_name_admin(user)] emagged cyborg [key_name_admin(src)].  Laws overridden.")
						...
						laws.show_laws(src)
						src << "<span class='warning'>\b ALERT: [user.real_name] is your new master. Obey your new laws and his commands.</span>"
						updateicon()
					else

	...
	if(ishuman(usr) || ismonkey(usr) || isrobot(usr) ||  isalienadult(usr))
		...
		//Adding things to inventory
			...
			if(!usr.get_active_hand())
				usr << "<span class='info'>You have nothing in your hand to put on its [add_to].</span>"
				return
			switch(add_to)
				if("ears")
					if(ears)
						...
						if( !istype(item_to_add,  /obj/item/device/radio/headset) )
							usr << "<span class='info'>This object won't fit.</span>"
							return
...
/mob/living/simple_animal/parrot/attackby(var/obj/item/O as obj, var/mob/living/user as mob)
	if(!stat && !client && !istype(O, /obj/item/stack/medical) && !istype(O,/obj/item/weapon/reagent_containers/food/snacks/cracker))
		...
		if(health < maxHealth)
			adjustBruteLoss(-10)
		user << "<span class='info'>[src] eagerly devours the cracker.</span>"
	..()
	return
		...
		if(in_range(src, parrot_interest))
			...
			if(isliving(parrot_interest))
				...
				if(!parrot_perch || parrot_interest.loc != parrot_perch.loc)
					...
					visible_message("[src] grabs [held_item]!", "<span class='info'>You grab [held_item]!</span>", "You hear the sounds of wings flapping furiously.")
...
/mob/living/simple_animal/parrot/proc/steal_from_ground()
	...
	if(held_item)
		src << "<span class='info'>You are already holding [held_item]</span>"
		return 1
	...
	src << "<span class='info'>There is nothing of interest to take.</span>"
	return 0
...
/mob/living/simple_animal/parrot/proc/steal_from_mob()
	...
	for(var/mob/living/carbon/C in view(1,src))
		...
		if(stolen_item)
			...
			stolen_item.loc = src
			visible_message("[src] grabs [held_item] out of [C]'s hand!", "<span class='info'>You snag [held_item] out of [C]'s hand!</span>", "You hear the sounds of wings flapping furiously.")
			return held_item
...
/mob/living/simple_animal/parrot/proc/drop_held_item(var/drop_gently = 1)
	...
	if(!held_item)
		if(src == usr) //So that other mobs wont make this message appear when they're bludgeoning you.
			src << "<span class='info'>You have nothing to drop!</span>"
		return 0
		...
		for(var/atom/movable/AM in view(src,1))
			for(var/perch_path in desired_perches)
				if(istype(AM, perch_path))
					...
					icon_state = "parrot_sit"
					return
	src << "<span class='info'>There is no perch nearby to sit on.</span>"
	return

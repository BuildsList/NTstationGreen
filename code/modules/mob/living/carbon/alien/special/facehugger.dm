...
/obj/item/clothing/mask/facehugger/examine()
	...
	switch(stat)
		if(DEAD,UNCONSCIOUS)
			usr << "<span class='warning'>\b [src] is not moving.</span>"
		if(CONSCIOUS)
			usr << "<span class='warning'>\b [src] seems to be active.</span>"
	if (sterile)
		usr << "<span class='warning'>\b It looks like the proboscis has been removed.</span>"
	return
...
/obj/item/clothing/mask/facehugger/proc/Attach(M as mob)
	...
	L.visible_message("<span class='warning'>\b [src] leaps at [L]'s face!</span>")

	if(ishuman(L))
		var/mob/living/carbon/human/H = L
		if(H.head && H.head.flags & HEADCOVERSMOUTH)
			H.visible_message("<span class='warning'>\b [src] smashes against [H]'s [H.head]!</span>")
			Die()
			return 0
	...
	if(iscarbon(M))
		...
		if(target.wear_mask)
			...
			target.visible_message("<span class='warning'>\b [src] tears [W] off of [target]'s face!</span>")

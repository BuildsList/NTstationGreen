...
/obj/item/alien_embryo/process()
	...
	switch(stage)
		if(2, 3)
			...
			if(prob(1))
				affected_mob << "<span class='info'>Your throat feels sore.</span>"
			if(prob(1))
				affected_mob << "<span class='info'>Mucous runs down the back of your throat.</span>"
		if(4)
			if(prob(1))
				affected_mob.emote("sneeze")
			if(prob(1))
				affected_mob.emote("cough")
			if(prob(2))
				affected_mob << "<span class='info'>Your muscles ache.</span>"
				if(prob(20))
					affected_mob.take_organ_damage(1)
			if(prob(2))
				affected_mob << "<span class='info'>Your stomach hurts.</span>"
				if(prob(20))
					affected_mob.adjustToxLoss(1)
					affected_mob.updatehealth()
		if(5)
			affected_mob << "<span class='info'>You feel something tearing its way out of your stomach...</span>"

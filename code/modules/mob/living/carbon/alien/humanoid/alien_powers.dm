...
/mob/living/carbon/alien/humanoid/proc/neurotoxin() // ok
	...
	if(powerc(50))
		adjustToxLoss(-50)
		src.visible_message("<span class='info'>[src] spits neurotoxin!</span>", "\green You spit neurotoxin.")
...
/mob/living/carbon/alien/humanoid/proc/resin()
	...
	if(powerc(75))
		...
		for(var/mob/O in viewers(src, null))
			O.show_message(text("<span class='info'><B>[src] vomits up a thick purple substance and begins to shape it!</B></span>"), 1)
		switch(choice)
			if("resin door")

...
/mob/living/carbon/alien/humanoid/blob_act()
	...
	show_message("<span class='info'>The blob attacks!</span>")
	...
	if (health > -100)
		...
		if(M.powerlevel > 0)
			...
			if(prob(stunprob))
				...
				for(var/mob/O in viewers(src, null))
					if ((O.client && !( O.blinded )))
						O.show_message(text("<span class='info'><B>The [M.name] has shocked []!</B></span>", src), 1)
	...
	switch(M.a_intent)
		...
		if ("harm")
			...
			if (prob(90))
				...
				for(var/mob/O in viewers(src, null))
					if ((O.client && !( O.blinded )))
						O.show_message(text("<span class='info'><B>[] has punched []!</B></span>", M, src), 1)
				if ((stat != DEAD) && (damage > 9||prob(5)))//Regular humans have a very small chance of weakening an alien.
					Weaken(1,5)
					for(var/mob/O in viewers(M, null))
						if ((O.client && !( O.blinded )))
							O.show_message(text("<span class='info'><B>[] has weakened []!</B></span>", M, src), 1, "<span class='info'>You hear someone fall.</span>", 2)
				adjustBruteLoss(damage)
				updatehealth()
			else
				...
				for(var/mob/O in viewers(src, null))
					if ((O.client && !( O.blinded )))
						O.show_message(text("<span class='info'><B>[] has attempted to punch []!</B></span>", M, src), 1)
		...
		if ("disarm")
			if (!lying)
				if (prob(5))//Very small chance to push an alien down.
					...
					for(var/mob/O in viewers(src, null))
						if ((O.client && !( O.blinded )))
							O.show_message(text("<span class='info'><B>[] has pushed down []!</B></span>", M, src), 1)
				else
					if (prob(50))
						drop_item()
						playsound(loc, 'sound/weapons/thudswoosh.ogg', 50, 1, -1)
						for(var/mob/O in viewers(src, null))
							if ((O.client && !( O.blinded )))
								O.show_message(text("<span class='info'><B>[] has disarmed []!</B></span>", M, src), 1)
					else
						playsound(loc, 'sound/weapons/punchmiss.ogg', 25, 1, -1)
						for(var/mob/O in viewers(src, null))
							if ((O.client && !( O.blinded )))
								O.show_message(text("<span class='info'><B>[] has attempted to disarm []!</B></span>", M, src), 1)
	return
...
/mob/living/carbon/alien/humanoid/attack_alien(mob/living/carbon/alien/humanoid/M as mob)
	...
	switch(M.a_intent)
		...
		if ("help")
			...
			for(var/mob/O in viewers(src, null))
				if ((O.client && !( O.blinded )))
					O.show_message(text("<span class='info'>[M.name] nuzzles [] trying to wake it up!</span>", src), 1)

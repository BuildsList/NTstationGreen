...
/mob/living/carbon/alien/larva/blob_act()
	...
	show_message("<span class='info'>The blob attacks you!</span>")
	...
	switch(M.a_intent)
		...
		if ("grab")
			...
			if (prob(90))
				...
				for(var/mob/O in viewers(src, null))
					if ((O.client && !( O.blinded )))
						O.show_message(text("<span class='info'><B>[] has kicked []!</B></span>", M, src), 1)
				...
				for(var/mob/O in viewers(src, null))
					if ((O.client && !( O.blinded )))
						O.show_message(text("<span class='info'><B>[] has attempted to kick []!</B></span>", M, src), 1)
	return

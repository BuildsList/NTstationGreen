...
/mob/living/carbon/alien/larva/verb/evolve()
	...
	if(handcuffed || legcuffed)
		src << "<span class='info'>You cannot evolve when you are cuffed.</span>"

	if(amount_grown >= max_grown)	//TODO ~Carn
		//green is impossible to read, so i made these blue and changed the formatting slightly
		src << "<span class='info'><b>You are growing into a beautiful alien! It is time to choose a caste.</b></span>"
		src << "<span class='info'>There are three to choose from:</span>"
		...
		src << "<span class='info'>You are not fully grown.</span>"
		return

		...
		if (radiation)
			if (radiation > 100)
				...
				if(!container)//If it's not in an MMI
					...
					src << "<span class='info'>STATUS: CRITICAL AMOUNTS OF RADIATION DETECTED.</span>"
				...
				if(50 to 74)
					...
					if(prob(5))
						...
						if(!container)
							...
							src << "<span class='info'>STATUS: DANGEROUS LEVELS OF RADIATION DETECTED.</span>"
					updatehealth()
				...
				switch(emp_damage)
					...
					if(21 to 30)//High level of EMP damage, unable to see, hear, or speak
						...
						if(!alert)//Sounds an alarm, but only once per 'level'
							emote("alarm")
							src << "<span class='info'>Major electrical distruption detected: System rebooting.</span>"
							alert = 1
						if(prob(75))
							emp_damage -= 1
					if(20)
					...
					if(11 to 19)//Moderate level of EMP damage, resulting in nearsightedness and ear damage
						...
						if(!alert)
							emote("alert")
							src << "<span class='info'>Primary systems are now online.</span>"
							alert = 1
						if(prob(50))
							emp_damage -= 1
					if(10)
					...
					if(2 to 9)//Low level of EMP damage, has few effects(handled elsewhere)
						if(!alert)
							emote("notice")
							src << "<span class='info'>System reboot nearly complete.</span>"
							alert = 1
						if(prob(25))
							emp_damage -= 1
					if(1)
						alert = 0
						src << "<span class='info'>All systems restored.</span>"
						emp_damage -= 1

/mob/living/carbon/human/can_equip(obj/item/I, slot, disable_warning = 0)
	...
	switch(slot)
		...
		if(slot_belt)
			...
			if(!w_uniform)
				if(!disable_warning)
					src << "<span class='info'>You need a jumpsuit before you can attach this [I.name].</span>"
				return 0
		...
		if(slot_s_store)
			...
			if(!wear_suit)
				if(!disable_warning)
					src << "<span class='info'>You need a suit before you can attach this [I.name].</span>"
				return 0
		...
		if(H.equip_to_appropriate_slot(I))
			...
			if(istype(S, /obj/item/weapon/storage) && S.can_be_inserted(I,1))		//else we put in belt
				...
				if(istype(S, /obj/item/weapon/storage) && S.can_be_inserted(I,1))
					...
					H << "<span class='info'>You are unable to equip that.</span>"
...
/mob/living/carbon/human/equip_to_slot(obj/item/I, slot, redraw_mob = 1)
	...
	switch(slot)
		...
		if(slot_in_backpack)
			...
			src << "<span class='info'>You are trying to equip this item to an unsupported inventory slot. Report this to a coder!</span>"
			return

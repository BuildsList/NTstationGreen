	...
	proc/handle_disabilities()
		if (disabilities & EPILEPSY)
			if ((prob(1) && paralysis < 1))
				src << "\red Ó âàñ ïğèïàäîê!"
				for(var/mob/O in viewers(src, null))
					if(O == src)
						continue
					O.show_message(text("\red <B>[src] áüåòñ&#255; â ïğèïàäêå!"), 1)
				Paralyse(10)
				Jitter(1000)

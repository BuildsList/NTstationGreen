...
/mob/living/carbon/human
	...
	proc/handle_disabilities()
		if (disabilities & EPILEPSY)
			if ((prob(1) && paralysis < 1))
				src << "<span class='info'>Ó âàñ ïğèïàäîê!</span>"
				for(var/mob/O in viewers(src, null))
					if(O == src)
						continue
					O.show_message(text("<span class='info'><B>[src] áüåòñ&#255; â ïğèïàäêå!</span>"), 1)
				Paralyse(10)
				Jitter(1000)
			...
			if ((prob(10) && paralysis <= 1))
				...
				switch(rand(1, 3))
					if(1)
						emote("äåğãàåòñ&#255;")
					if(2 to 3)
						say("[prob(50) ? ";" : ""][pick("ÅÁÀÍÛÉ", "ÍÀÕÓÉ", "ÁËßÄÜ", "ØËŞÕÀ", "ÕÓÅÑÎÑ", "ÏÈÇÄÀ", "ÑÓÊÀ")]")
	...
	proc/handle_mutations_and_radiation()
		...
		if (has_organic_effect(/datum/organic_effect/hulk) && health <= 25)
			...
			src << "<span class='info'>You suddenly feel very weak.</span>"
			Weaken(3)
			emote("collapse")
		...
		if (radiation)
			if (radiation > 100)
				radiation = 100
				Weaken(10)
				src << "<span class='info'>You feel weak.</span>"
				emote("collapse")
			...
			if (radiation < 0)
				...
				switch(radiation)
					...
					if(75 to 100)
						...
						adjustToxLoss(3)
						if(prob(1))
							src << "<span class='info'>You mutate!</span>"
							randmutb(src)
							domutcheck(src,null)
							emote("gasp")
						updatehealth()
	...
	proc/handle_breath(datum/gas_mixture/breath)
		...
		if( (abs(310.15 - breath.temperature) > 50) && !has_organic_effect(/datum/organic_effect/cold_res)) // Hot air hurts :(
			if(breath.temperature < 260.15)
				if(prob(20))
					src << "<span class='info'>You feel your face freezing and an icicle forming in your lungs!</span>"
			else if(breath.temperature > 360.15)
				if(prob(20))
					src << "<span class='info'>You feel your face burning and a searing heat in your lungs!</span>"
	...
	proc/handle_chemicals_in_body()
		...
		if(has_organic_effect(/datum/organic_effect/fat))
			...
			if(overeatduration > 500)
				src << "<span class='info'>You suddenly feel blubbery!</span>"

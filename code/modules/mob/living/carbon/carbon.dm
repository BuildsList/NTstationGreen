	...
	if(user in src.stomach_contents)
		if(prob(40))
			for(var/mob/M in hearers(4, src))
				if(M.client)
					M.show_message(text("<span class='info'>You hear something rumbling inside [src]'s stomach...</span>"), 2)
			...
			if(I && I.force)
				...
				for(var/mob/M in viewers(user, null))
					if(M.client)
						M.show_message(text("<span class='info'><B>[user] attacks [src]'s stomach wall with the [I.name]!</span>"), 2)
...
/mob/living/carbon/throw_item(atom/target)
	...
	if(item)
		...
		src.visible_message("<span class='info'>[src] has thrown [item].</span>")
...
/mob/living/carbon/proc/handle_ventcrawl(var/obj/machinery/atmospherics/unary/vent_pump/vent_found = null) // -- TLE -- Merged by Carn
	...
	if(!vents.len)
		src << "<span class='info'>There are no available vents to travel to, they could be welded.</span>"
		return
	...
	for(var/obj/item/carried_item in contents)//If the monkey got on objects.
		if( !istype(carried_item, /obj/item/weapon/implant) && !istype(carried_item, /obj/item/clothing/mask/facehugger) )//If it's not an implant or a facehugger
			src << "<span class='info'>You can't be carrying items or have items equipped when vent crawling!</span>"
			return
	if(isslime(src))
		var/mob/living/carbon/slime/S = src
		if(S.Victim)
			src << "<span class='info'>You'll have to let [S.Victim] go or finish eating \him first.</span>"
			return
	...
	spawn(travel_time)
		...
		if(target_vent.welded)			//the vent can be welded while alien scrolled through the list or travelled.
			...
			src << "<span class='info'>The vent you were heading to appears to be welded.</span>"
		loc = target_vent.loc

...
/mob/proc/Cell()
	...
	var/t = "<span class='info'>Coordinates: [x],[y] \n</span>"
	t+= "<span class='info'>Temperature: [environment.temperature] \n</span>"
	t+= "<span class='info'>Nitrogen: [environment.nitrogen] \n</span>"
	t+= "<span class='info'>Oxygen: [environment.oxygen] \n</span>"
	t+= "<span class='info'>Plasma : [environment.toxins] \n</span>"
	t+= "<span class='info'>Carbon Dioxide: [environment.carbon_dioxide] \n</span>"
	for(var/datum/gas/trace_gas in environment.trace_gases)
		usr << "<span class='info'>[trace_gas.type]: [trace_gas.moles] \n</span>"
...
/mob/proc/equip_to_slot_if_possible(obj/item/W as obj, slot, qdel_on_fail = 0, disable_warning = 0, redraw_mob = 1)
	...
	if(!W.mob_can_equip(src, slot, disable_warning))
		if(qdel_on_fail)
			...
			if(!disable_warning)
				src << "<span class='info'>You are unable to equip that.</span>" //Only print if qdel_on_fail is false
		return 0
...
/mob/verb/abandon_mob()
	...
	if (!( abandon_allowed ))
		return
	if ((stat != 2 || !( ticker )))
		usr << "<span class='info'><B>You must be dead to use this!</B></span>"
		return

	log_game("[usr.name]/[usr.key] used abandon mob.")

	usr << "<span class='info'><B>Please roleplay correctly!</B></span>"
...
/mob/verb/observe()
	...
	if(check_rights_for(client,R_ADMIN))
		...
		usr << "<span class='info'>You must be observing to use this!</span>"
		return

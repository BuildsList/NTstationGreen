...
/obj/machinery/power/rad_collector/attackby(obj/item/W, mob/user)
	if(istype(W, /obj/item/device/multitool))
		user << "<span class='info'>The [W.name] detects that [last_power]W were recently produced.</span>"
		return 1
	else if(istype(W, /obj/item/device/analyzer) && P)
		atmosanalyzer_scan(P.air_contents, user)
	else if(istype(W, /obj/item/weapon/tank/plasma))
		if(!src.anchored)
			user << "<span class='info'>The [src] needs to be secured to the floor first.</span>"
			return 1
		if(src.P)
			user << "<span class='info'>There's already a plasma tank loaded.</span>"
			return 1
		user.drop_item()
		src.P = W
		W.loc = src
		...
		if(P)
			user << "<span class='info'>Remove the plasma tank first.</span>"
			return 1
		...
		if (src.allowed(user))
			if(active)
				...
				user << "<span class='info'>The controls can only be locked when the [src] is active</span>"
		else

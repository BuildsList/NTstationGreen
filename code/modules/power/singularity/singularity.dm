		...
		if(M.stat == CONSCIOUS)
			if (istype(M,/mob/living/carbon/human))
				...
				if(istype(H.glasses,/obj/item/clothing/glasses/meson))
					H << "<span class='info'>You look directly into The [src.name], good thing you had your protective eyewear on!</span>"
					return
		M << "<span class='info'>You look directly into The [src.name] and feel weak.</span>"
		M.apply_effect(3, STUN)
		for(var/mob/O in viewers(M, null))
			O.show_message(text("<span class='info'><B>[] stares blankly at The []!</B></span>", M, src), 1)
	return
...
/obj/machinery/singularity/narsie/mezzer()
	for(var/mob/living/carbon/M in oviewers(8, src))
		if(M.stat == CONSCIOUS)
			if(!iscultist(M))
				M << "<span class='info'>You feel your sanity crumble away in an instant as you gaze upon [src.name]...</span>"
				M.apply_effect(3, STUN)
...
/obj/machinery/singularity/narsie/proc/acquire(var/mob/food)
	target << "<span class='info'><b>NAR-SIE HAS LOST INTEREST IN YOU</b></span>"
	target = food
	if(ishuman(target))
		target << "<span class='info'><b>NAR-SIE HUNGERS FOR YOUR SOUL</b></span>"
	else
		target << "<span class='info'><b>NAR-SIE HAS CHOSEN YOU TO LEAD THEM TO THEIR NEXT MEAL</b></span>"

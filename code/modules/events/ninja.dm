...
/obj/item/clothing/suit/space/space_ninja/proc/lock_suit(mob/living/carbon/U, X = 0)
	if(X)//If you want to check for icons.
		...
		if(U.mind.special_role!="Space Ninja")
			U << "<span class='info'><B>fÄTaL ÈÈRRoR</B>: 382200-*#00CÖDE <B>RED</B>\nUNAU†HORIZED USÈ DETÈC†††eD\nCoMMÈNCING SUB-R0U†IN3 13...\nTÈRMInATING U-U-USÈR...</span>"
			U.gib()
			return 0
		if(!istype(U:head, /obj/item/clothing/head/helmet/space/space_ninja))
			U << "<span class='info'><B>ERROR</B>: 100113 \black UNABLE TO LOCATE HEAD GEAR\nABORTING...</span>"
			return 0
		if(!istype(U:shoes, /obj/item/clothing/shoes/space_ninja))
			U << "<span class='info'><B>ERROR</B>: 122011 \black UNABLE TO LOCATE FOOT GEAR\nABORTING...</span>"
			return 0
		if(!istype(U:gloves, /obj/item/clothing/gloves/space_ninja))
			U << "<span class='info'><B>ERROR</B>: 110223 \black UNABLE TO LOCATE HAND GEAR\nABORTING...</span>"
			return 0
...
/obj/item/clothing/suit/space/space_ninja/proc/grant_kamikaze(mob/living/carbon/U)
	...
	U << "<span class='info'>Do or Die, <b>LET'S ROCK!!</b></span>"
...
/obj/item/clothing/gloves/space_ninja/proc/drain_wire()
	...
	if(candrain&&!draining)
		...
		if(isturf(T) && T.is_plating())
			...
			if(!attached)
				U << "<span class='info'>Warning: no exposed cable available.</span>"
			else
				U << "<span class='info'>Connecting to wire, stand still...</span>"
				if(do_after(U,50)&&!isnull(attached))
					drain("WIRE",attached,U:wear_suit,src)
				else
					U << "<span class='info'>Procedure interrupted. Protocol terminated.</span>"
	return
...
/obj/item/clothing/suit/space/space_ninja/proc/ninjacost(C = 0,X = 0)
	...
	if( (U.stat||U.incorporeal_move)&&X!=3 )//Will not return if user is using an adrenaline booster since you can use them when stat==1.
		U << "<span class='info'>You must be conscious and solid to do this.</span>"//It's not a problem of stat==2 since the ninja will explode anyway if they die.
		return 1
	else if(C&&cell.charge<C*10)
		U << "<span class='info'>Not enough energy.</span>"
		return 1
	switch(X)
		if(1)
		...
		if(2)
			if(s_bombs<=0)
				U << "<span class='info'>There are no more smoke bombs remaining.</span>"
				return 1
		if(3)
			if(a_boost<=0)
				U << "<span class='info'>You do not have any more adrenaline boosters.</span>"
				return 1
...
/obj/item/clothing/suit/space/space_ninja/proc/ninjasmoke()
	...
	if(!ninjacost(,2))
		var/mob/living/carbon/human/U = affecting
		U << "<span class='info'>There are <B>[s_bombs]</B> smoke bombs remaining.</span>"
...
/obj/item/clothing/suit/space/space_ninja/proc/ninjajaunt()
	...
	if(!ninjacost(C,1))
		...
		if(destination&&istype(mobloc, /turf))//The turf check prevents unusual behavior. Like teleporting out of cryo pods, cloners, mechs, etc.
			...
			U << "<span class='info'>The VOID-shift device is malfunctioning, <B>teleportation failed</B>.</span>"
	return
...
/obj/item/clothing/suit/space/space_ninja/proc/ninjashift(turf/T in oview())
	...
	if(!ninjacost(C,1))
		...
		if((!T.density)&&istype(mobloc, /turf))
			...
			U << "<span class='info'>You cannot teleport into solid walls or from solid matter</span>"
	return
	...
	if(!ninjacost(C))
		...
		if(!kamikaze)
			if(!U.get_active_hand()&&!istype(U.get_inactive_hand(), /obj/item/weapon/melee/energy/blade))
				...
				U << "<span class='info'>You can only summon one blade. Try dropping an item first.</span>"
...
/obj/item/clothing/suit/space/space_ninja/proc/ninjastar()
	...
	if(!ninjacost(C))
		...
		if(targets.len)
			...
			U << "<span class='info'>There are no targets in view.</span>"
	return
		...
		//if(M)//DEBUG
			if(!locate(/obj/effect/energy_net) in M.loc)//Check if they are already being affected by an energy net.
				...
				for(var/mob/O in viewers(U, 3))
					O.show_message(text("<span class='info'>[] caught [] with an energy net!</span>", U, M), 1)
				E.affecting = M
				E.master = U
				spawn(0)//Parallel processing.
					E.process(M)
...
/obj/item/clothing/suit/space/space_ninja/proc/ninjaboost()
	...
	*/
		...
		spawn(70)
			...
			U << "<span class='info'>You are beginning to feel the after-effect of the injection.</span>"
		a_boost--
		s_coold = 3
	return
...
/obj/item/clothing/suit/space/space_ninja/proc/ninjawalk()
	...
	if(!U.incorporeal_move)
		...
		U << "<span class='info'>You will now phase through solid matter.</span>"
	else
		U.incorporeal_move = 0
		U << "<span class='info'>You will no-longer phase through solid matter.</span>"
	return
...
/obj/item/clothing/suit/space/space_ninja/proc/killai(mob/living/silicon/ai/A = AI)
	if(A.client)
		A << "<span class='info'>Self-erase protocol dete-- *bzzzzz*</span>"
...
/obj/item/clothing/suit/space/space_ninja/proc/deinit()
	...
	if(s_control&&!s_busy)
		...
		affecting << "<span class='info'>The function did not trigger!</span>"
	return
...
/obj/item/clothing/suit/space/space_ninja/proc/spideros()
	...
	if(s_control&&!s_busy&&!kamikaze)
		...
		affecting << "<span class='info'>The interface is locked!</span>"
	return
...
/obj/item/clothing/suit/space/space_ninja/proc/stealth()
	...
	if(s_control&&!s_busy)
		...
		affecting << "<span class='info'>Stealth does not appear to work!</span>"
	return
...
/obj/item/clothing/suit/space/space_ninja/proc/ninitialize(delay = s_delay, mob/living/carbon/human/U = loc)
	if(U.mind && U.mind.assigned_role=="MODE" && !s_initialized && !s_busy)//Shouldn't be busy... but anything is possible I guess.
		...
		for(var/i,i<7,i++)
			switch(i)
				if(0)
					U << "<span class='info'>Now initializing...</span>"
				if(1)
					if(!lock_suit(U))//To lock the suit onto wearer.
						break
					U << "<span class='info'>Securing external locking mechanism...\nNeural-net established.</span>"
				if(2)
					U << "<span class='info'>Extending neural-net interface...\nNow monitoring brain wave pattern...</span>"
				if(3)
					if(U.stat==2||U.health<=0)
						U << "<span class='info'><B>FÄ†AL ï¿½Rrï¿½R</B>: 344--93#ï¿½&&21 BRï¿½ï¿½N |/|/aVï¿½ PATT$RN <B>RED</B>\nA-A-aBï¿½rTï¿½NG...</span>"
						unlock_suit()
						break
					lock_suit(U,1)//Check for icons.
					U.regenerate_icons()
					U << "<span class='info'>Linking neural-net interface...\nPattern \green <B>GREEN</B>\blue, continuing operation.</span>"
				if(4)
					U << "<span class='info'>VOID-shift device status: <B>ONLINE</B>.\nCLOAK-tech device status: <B>ONLINE</B>.</span>"
				if(5)
					U << "<span class='info'>Primary system status: <B>ONLINE</B>.\nBackup system status: <B>ONLINE</B>.\nCurrent energy capacity: <B>[cell.charge]</B>.</span>"
				if(6)
					U << "<span class='info'>All systems operational. Welcome to <B>SpiderOS</B>, [U.real_name].</span>"
					grant_ninja_verbs()
					grant_equip_verbs()
					ntick()
			sleep(delay)
		...
		if(!U.mind||U.mind.assigned_role!="MODE")//Your run of the mill persons shouldn't know what it is. Or how to turn it on.
			...
			U << "<span class='info'>The suit is already functioning. \black <b>Please report this bug.</b></span>"
		else
			U << "<span class='info'><B>ERROR</B>: \black You cannot use this function at this time.</span>"
	return
...
/obj/item/clothing/suit/space/space_ninja/proc/deinitialize(delay = s_delay)
	if(affecting==loc&&!s_busy)
		var/mob/living/carbon/human/U = affecting
		if(!s_initialized)
			U << "<span class='info'>The suit is not initialized. \black <b>Please report this bug.</b></span>"
			return
		...
		for(var/i = 0,i<7,i++)
			switch(i)
				if(0)
					U << "<span class='info'>Now de-initializing...</span>"
					remove_kamikaze(U)//Shutdowns kamikaze.
					spideros = 0//Spideros resets.
				if(1)
					U << "<span class='info'>Logging off, [U:real_name]. Shutting down <B>SpiderOS</B>.</span>"
					remove_ninja_verbs()
				if(2)
					U << "<span class='info'>Primary system status: <B>OFFLINE</B>.\nBackup system status: <B>OFFLINE</B>.</span>"
				if(3)
					U << "<span class='info'>VOID-shift device status: <B>OFFLINE</B>.\nCLOAK-tech device status: <B>OFFLINE</B>.</span>"
					cancel_stealth()//Shutdowns stealth.
				if(4)
					U << "<span class='info'>Disconnecting neural-net interface...\green<B>Success</B>\blue.</span>"
				if(5)
					U << "<span class='info'>Disengaging neural-net interface...\green<B>Success</B>\blue.</span>"
				if(6)
					U << "<span class='info'>Unsecuring external locking mechanism...\nNeural-net abolished.\nOperation status: <B>FINISHED</B>.</span>"
					blade_check(U,2)
					remove_equip_verbs()
					unlock_suit()
					U.regenerate_icons()
...
/obj/item/clothing/suit/space/space_ninja/Topic(href, href_list)
	...
	if(s_control)
		if(!affecting||U.stat||!s_initialized)//Check to make sure the guy is wearing the suit after clicking and it's on.
			U << "<span class='info'>Your suit must be worn and active to use this function.</span>"
		...
		if(!affecting||A.stat||!s_initialized||A.loc!=src)
			A << "<span class='info'>This function is not available at this time.</span>"
	...
	switch(href_list["choice"])
		...
		if("Shock")
			...
			if(damage>1)//So they don't spam it when energy is a factor.
				...
				A << "<span class='info'><b>ERROR</b>: \black Not enough energy remaining.</span>"
		...
		if("Message")
			...
			if(isnull(P)||P.toff)//So it doesn't freak out if the object no-longer exists.
				display_to << "<span class='info'>Error: unable to deliver message.</span>"
				display_spideros()
				return
		...
		if("Inject")
			if( (href_list["tag"]=="radium"? (reagents.get_reagent_amount("radium"))<=(a_boost*a_transfer) : !reagents.get_reagent_amount(href_list["tag"])) )//Special case for radium. If there are only a_boost*a_transfer radium units left.
				display_to << "<span class='info'>Error: the suit cannot perform this function. Out of [href_list["name"]].</span>"
			else
				reagents.reaction(U, 2)
				...
				if( !(U.stat||U.wear_suit!=src||!s_initialized) )
					if( !(cell.charge<=1||s_busy) )
						...
						for(var/i, i<4, i++)
							switch(i)
								if(0)
									U << "<span class='info'>Engaging mode...\n\black<b>CODE NAME</b>: \red <b>KAMIKAZE</b></span>"
								if(1)
									U << "<span class='info'>Re-routing power nodes... \nUnlocking limiter...</span>"
								if(2)
									U << "<span class='info'>Power nodes re-routed. \nLimiter unlocked.</span>"
								if(3)
									...
									message_admins("<span class='info'>[key_name_admin(U)] used KAMIKAZE mode.</span>")//Let the admins know.
									s_busy = 0
									return
							sleep(s_delay)
					else
						U << "<span class='info'><b>ERROR<b>: \black Unable to initiate mode.</span>"
				else
					U << browse(null, "window=spideros")
					s_busy = 0
					return
			else
				U << "<span class='info'>ERROR: WRONG PASSWORD!</span>"
				k_unlock = 0
				spideros = 0
			s_busy = 0
		...
		if("Eject Disk")
			...
			if(!U.get_active_hand())
				...
				if(T)
					...
					U << "<span class='info'><b>ERROR<b>: \black Could not eject disk.</span>"
		...
		if("Eject pAI")
			...
			if(!U.get_active_hand())
				...
				if(T)
					...
					U << "<span class='info'><b>ERROR<b>: \black Could not eject pAI card.</span>"
		...
		if("Override AI Laws")
			...
			A.show_laws()
			U << "<span class='info'>Law Override: <b>SUCCESS</b>.</span>"
				...
				if(A.laws.zeroth)//Gives a few seconds to re-upload the AI somewhere before it takes full control.
					...
					for(var/i,i<5,i++)
						if(AI==A)
							switch(i)
								if(0)
									A << "<span class='info'><b>WARNING</b>: \black purge procedure detected. \nNow hacking host...</span>"
									U << "<span class='info'><b>WARNING</b>: HACKING ATï¿½ï¿½TEMPï¿½ IN PR0GRESs!</span>"
									spideros = 0
									k_unlock = 0
									U << browse(null, "window=spideros")
								if(1)
									A << "Disconnecting neural interface..."
									U << "<span class='info'><b>WARï¿½NING</b>: ï¿½Rï¿½O0ï¿½Grï¿½--S 2&3%</span>"
								if(2)
									A << "Shutting down external protocol..."
									U << "<span class='info'><b>WARNING</b>: Pï¿½ï¿½ï¿½ï¿½RÖ†Grï¿½5S 677^%</span>"
									cancel_stealth()
								if(3)
									A << "Connecting to kernel..."
									U << "<span class='info'><b>WARNING</b>: ï¿½Rï¿½rï¿½R_404</span>"
									A.control_disabled = 0
								if(4)
									A << "Connection established and secured. Menu updated."
									U << "<span class='info'><b>Wï¿½r#nING</b>: #%@!!WÈ†|_4ï¿½54@ \nUnï¿½B88l3 Tï¿½ Lï¿½-ï¿½o-Lï¿½CaT2 ##$!ï¿½RNï¿½0..%..</span>"
									grant_AI_verbs()
									return
							sleep(s_delay)
						else	break
					s_busy = 0
					U << "<span class='info'>Hacking attempt disconnected. Resuming normal operation.</span>"
				else
					flush = 1
					A.suiciding = 1
...
/obj/item/clothing/suit/space/space_ninja/proc/ai_holo(var/turf/T in oview(3,affecting))//To have an internal AI display a hologram to the AI and ninja only.
	...
	if(s_initialized&&affecting&&affecting.client&&istype(affecting.loc, /turf))//If the host exists and they are playing, and their location is a turf.
		if(!hologram)//If there is not already a hologram.
			...
			AI << "<span class='info'>ERROR: \black Image feed in progress.</span>"
	else
		AI << "<span class='info'>ERROR: \black Unable to project image.</span>"
	return
	...
	if(U==affecting)//Safety, in case you try doing this without wearing the suit/being the person with the suit.
		if(istype(I, /obj/item/device/aicard))//If it's an AI card.
			if(s_control)
				...
				U << "<span class='info'><b>ERROR</b>: \black Remote access channel disabled.</span>"
			...
			I.loc = src
			pai = I
			U << "<span class='info'>You slot \the [I] into \the [src].</span>"
			updateUsrDialog()
			return
			...
			if(I:maxcharge>cell.maxcharge&&n_gloves&&n_gloves.candrain)
				U << "<span class='info'>Higher maximum capacity detected.\nUpgrading...</span>"
				if (n_gloves&&n_gloves.candrain&&do_after(U,s_delay))
					...
					old_cell.updateicon()
					cell = I
					U << "<span class='info'>Upgrade complete. Maximum capacity: <b>[round(cell.maxcharge/100)]</b>%</span>"
				else
					U << "<span class='info'>Procedure interrupted. Protocol terminated.</span>"
			return
				...
				if(do_after(U,s_delay))
					for(var/datum/tech/current_data in stored_research)
						if(current_data.id==TD.stored.id)
							...
							break
					TD.stored = null
					U << "<span class='info'>Data analyzed and updated. Disk erased.</span>"
				else
					U << "<span class='info'><b>ERROR</b>: \black Procedure interrupted. Process terminated.</span>"
			else
				I.loc = src
				t_disk = I
				U << "<span class='info'>You slot \the [I] into \the [src].</span>"
...
/obj/item/clothing/suit/space/space_ninja/proc/toggle_stealth()
	...
	if(s_active)
		...
		s_active=!s_active
		U.alpha = 0
		U << "<span class='info'>You are now invisible to normal detection.</span>"
...
/obj/item/clothing/suit/space/space_ninja/proc/cancel_stealth()
	...
	if(s_active)
		...
		s_active=!s_active
		U.alpha = 255
		U << "<span class='info'>You are now visible.</span>"
		for(var/mob/O in oviewers(U))
	...
	if(s_initialized)
		...
		if(s_control)
			...
			if(!kamikaze)
				...
				U << "<span class='info'>KAMIKAZE MODE ENGAGED!</span>"
...
//Var Initialize
	...
	if(target_type!="RESEARCH")//I lumped research downloading here for ease of use.
		U << "<span class='info'>Now charging battery...</span>"
	...
	switch(target_type)
		...
		if("APC")
			...
			if(A.cell&&A.cell.charge)
				...
				U << "<span class='info'>Gained <B>[totaldrain]</B> energy from the APC.</span>"
				if(!A.emagged)
					flick("apc-spark", src)
					A.emagged = 1
					A.locked = 0
				...
				U << "<span class='info'>This APC has run dry of power. You must find another source.</span>"
		...
		if("SMES")
			...
			if(A.charge)
				...
				U << "<span class='info'>Gained <B>[totaldrain]</B> energy from the SMES cell.</span>"
			else
				U << "<span class='info'>This SMES cell has run dry of power. You must find another source.</span>"

		if("CELL")
			var/obj/item/weapon/stock_parts/cell/A = target
			if(A.charge)
				if (G.candrain&&do_after(U,30))
					U << "<span class='info'>Gained <B>[A.charge]</B> energy from the cell.</span>"
				...
				U << "<span class='info'>This cell is empty and of no use.</span>"
			...
			if(A.powered())//If powered.
				...
				if(B)//If APC exists. Might not if the area is unpowered like Centcom.
					...
					while(G.candrain&&!maxcapacity&&!isnull(A))//And start a proc similar to drain from wire.
						...
						spark_system.start()
						if(drained==0)	break
					U << "<span class='info'>Gained <B>[totaldrain]</B> energy from the power network.</span>"
				else
					U << "<span class='info'>Power network could not be found. Aborting.</span>"
			else
				U << "<span class='info'>This recharger is not providing energy. You must find another source.</span>"

		if("RESEARCH")
			var/obj/machinery/A = target
			U << "<span class='info'>Hacking \the [A]...</span>"
			spawn(0)
				var/turf/location = get_turf(U)
				for(var/mob/living/silicon/ai/AI in player_list)
					AI << "<span class='info'><b>Network Alert: Hacking attempt detected[location?" in [location]</span>":". Unable to pinpoint location"]</b>."
			if(A:files&&A:files.known_tech.len)
				for(var/datum/tech/current_data in S.stored_research)
					U << "<span class='info'>Checking \the [current_data.name] database.</span>"
						...
						for(var/datum/tech/analyzing_data in A:files.known_tech)
							if(current_data.id==analyzing_data.id)
								if(analyzing_data.level>current_data.level)
									U << "<span class='info'>Database: \black <b>UPDATED</b>.</span>"
									current_data.level = analyzing_data.level
								break//Move on to next.
					else	break//Otherwise, quit processing.
			U << "<span class='info'>Data analyzed. Process finished.</span>"
		...
		if("MECHA")
			...
			A.occupant_message("<span class='info'>Warning: Unauthorized access through sub-route 4, block H, detected.</span>")
			if(A.get_charge())
				while(G.candrain&&A.cell.charge>0&&!maxcapacity)
					drain = rand(G.mindrain,G.maxdrain)
					if(A.cell.charge<drain)
				...
				U << "<span class='info'>Gained <B>[totaldrain]</B> energy from [src].</span>"
			else
				U << "<span class='info'>The exosuit's battery has run dry. You must find another source of power.</span>"

		if("CYBORG")
			var/mob/living/silicon/robot/A = target
			A << "<span class='info'>Warning: Unauthorized access through sub-route 12, block C, detected.</span>"
			G.draining = 1
			if(A.cell&&A.cell.charge)
				while(G.candrain&&A.cell.charge>0&&!maxcapacity)
					drain = rand(G.mindrain,G.maxdrain)
				...
				U << "<span class='info'>Gained <B>[totaldrain]</B> energy from [A].</span>"
			else
				U << "<span class='info'>Their battery has run dry of power. You must find another source.</span>"
...
/obj/effect/energy_net
			...
			if(health <=0)
				...
				if(affecting)
					...
					if(!isnull(master))//As long as they still exist.
						master << "<span class='info'><b>ERROR</b>: \black unable to initiate transport protocol. Procedure terminated.</span>"
				qdel(src)
			return
	...
	process(var/mob/living/carbon/M as mob)
		...
		if(isnull(M)||M.loc!=loc)//If mob is gone or not at the location.
			if(!isnull(master))//As long as they still exist.
				master << "<span class='info'><b>ERROR</b>: \black unable to locate \the [mob_name]. Procedure terminated.</span>"
		...
		if(!isnull(src))//As long as both net and person exist.
			...
			M << "<span class='info'>You appear in a strange place!</span>"
			...
			if(!isnull(master))//As long as they still exist.
				master << "<span class='info'><b>SUCCESS</b>: \black transport procedure of \the [affecting] complete.</span>"

			M.anchored = 0//Important.

		else//And they are free.
			M << "<span class='info'>You are free of the net!</span>"
		return
	...
	hitby(AM as mob|obj)
		...
		for(var/mob/O in viewers(src, null))
			O.show_message(text("<span class='info'><B>[src] was hit by [AM].</B></span>"), 1)
		var/tforce = 0
		if(ismob(AM))
			tforce = 10
		else
	...
	attack_hand()
		if (usr.has_organic_effect(/datum/organic_effect/hulk))
			usr << text("<span class='info'>You easily destroy the energy net.</span>")
			for(var/mob/O in oviewers(src))
				O.show_message(text("<span class='info'>[] rips the energy net apart!</span>", usr), 1)
			health-=50
		healthcheck()
		return
	...
	attack_alien()
		...
		for(var/mob/O in oviewers(src))
			O.show_message(text("<span class='info'>[] claws at the energy net!</span>", usr), 1)
		playsound(src.loc, 'sound/weapons/slash.ogg', 80, 1)
		health -= rand(10, 20)
		if(health <= 0)
			usr << text("\green You slice the energy net to pieces.")
			for(var/mob/O in oviewers(src))
				O.show_message(text("<span class='info'>[] slices the energy net apart!</span>", usr), 1)
		healthcheck()
		return

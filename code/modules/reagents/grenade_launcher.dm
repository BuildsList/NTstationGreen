/obj/item/weapon/gun/grenadelauncher
	...
	attackby(obj/item/I as obj, mob/user as mob)
		...
		if((istype(I, /obj/item/weapon/grenade)))
			if(grenades.len < max_grenades)
				...
				usr << "<span class='info'>The grenade launcher cannot hold more grenades.</span>"
	...
	afterattack(obj/target, mob/user , flag)
		...
		if(grenades.len)
			...
			usr << "<span class='info'>The grenade launcher is empty.</span>"

	proc
		fire_grenade(atom/target, mob/user)
			for(var/mob/O in viewers(world.view, user))
				O.show_message(text("<span class='info'>[] fired a grenade!</span>", user), 1)
			user << "<span class='info'>You fire the grenade launcher!</span>"

...
/obj/item/weapon/reagent_containers/food/drinks/bottle/attack(mob/living/target as mob, mob/living/user as mob)
	...
	if(istype(target, /mob/living/carbon/))
		...
		for(var/mob/O in viewers(user, null))
			if(target != user) O.show_message(text("<span class='info'><B>[target] has been hit over the head with a bottle of [src.name], by [user]!</B></span>"), 1)
			else O.show_message(text("<span class='info'><B>[target] hit himself with a bottle of [src.name] on the head!</B></span>"), 1)
	...
	if(src.reagents)
		for(var/mob/O in viewers(user, null))
			O.show_message(text("<span class='info'><B>The contents of the [src] splashes all over [target]!</B></span>"), 1)

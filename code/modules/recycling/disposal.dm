		...
		if(ismob(G.affecting))
			...
			if(do_after(usr, 20))
				...
				for (var/mob/C in viewers(src))
					C.show_message("<span class='info'>[GM.name] has been placed in \the [src] by [user].</span>", 3)
				qdel(G)
...
/obj/machinery/disposal/attack_hand(mob/user as mob)
	if(user && user.loc == src)
		usr << "<span class='info'>You cannot reach the controls from inside.</span>"
		return
	/*
	if(mode==-1)
		usr << "<span class='info'>The disposal units power is disabled.</span>"
		return
	*/
	interact(user, 0)
...
/obj/machinery/disposal/Topic(href, href_list)
	...
	if(mode==-1 && !href_list["eject"]) // only allow ejecting if mode is -1
		usr << "<span class='info'>\The [src]'s power is disabled.</span>"
		return
	..()
	usr.set_machine(src)
	...
	proc/init(var/obj/machinery/disposal/D)
		...
		for(var/atom/movable/AM in D)
			...
			if(istype(AM, /mob/living/carbon/human))
				...
				if(H.has_organic_effect(/datum/organic_effect/fat))		// is a human and fat?

...
/obj/proc/atmosanalyzer_scan(var/datum/gas_mixture/air_contents, mob/user, var/obj/target = src)
	var/obj/icon = target
	user.visible_message("<span class='info'>[user] has used the analyzer on \icon[icon] [target].</span></span>")
	...
	if(total_moles>0)
		...
		if(unknown_concentration>0.01)
			user << "<span class='info'>Unknown: [round(unknown_concentration*100)]%</span>"

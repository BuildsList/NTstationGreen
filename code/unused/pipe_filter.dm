...
/obj/machinery/pipefilter/attackby(obj/item/weapon/W, mob/user as mob)
	...
	if(istype(W, /obj/item/weapon/screwdriver))
		if(bypassed)
			user.show_message(text("<span class='info'>Remove the foreign wires first!</span>"), 1)
			return
		src.add_fingerprint(user)
		user.show_message(text("<span class='info'>Now []securing the access system panel...</span>", (src.locked) ? "un" : "re"), 1)
		sleep(30)
		locked =! locked
		user.show_message(text("<span class='info'>Done!</span>"),1)
		src.updateicon()
		return
	if(istype(W, /obj/item/stack/cable_coil) && !bypassed)
		if(src.locked)
			user.show_message(text("<span class='info'>You must remove the panel first!</span>"),1)
			return
		var/obj/item/stack/cable_coil/C = W
		if(C.use(4))
			user.show_message(text("<span class='info'>You unravel some cable..</span>"),1)
		else
			user.show_message(text("<span class='info'>Not enough cable! <I>(Requires four pieces)</I></span>"),1)
		src.add_fingerprint(user)
		user.show_message(text("<span class='info'>Now bypassing the access system... <I>(This may take a while)</I></span>"), 1)
		sleep(100)
		bypassed = 1
		src.updateicon()
		return
	if(istype(W, /obj/item/weapon/wirecutters) && bypassed)
		src.add_fingerprint(user)
		user.show_message(text("<span class='info'>Now removing the bypass wires... <I>(This may take a while)</I></span>"), 1)
		sleep(50)
		bypassed = 0
		src.updateicon()
		return
	if(istype(W, /obj/item/weapon/card/emag) && (!emagged))
		...
		for(var/mob/O in viewers(user, null))
			O.show_message(text("<span class='info'>[] has shorted out the [] with an electromagnetic card!</span>", user, src), 1)
...
/obj/machinery/pipefilter/Topic(href, href_list)
	...
	if ((((get_dist(src, usr) <= 1 || usr.telekinesis == 1) || istype(usr, /mob/living/silicon/ai)) && istype(src.loc, /turf)))
		...
		if (src.allowed(usr) || src.emagged || src.bypassed)
			...
			usr.see("<span class='info'>Access Denied ([src.name] operation restricted to authorized atmospheric technicians.)</span>")
		AutoUpdateAI(src)
		src.updateUsrDialog()

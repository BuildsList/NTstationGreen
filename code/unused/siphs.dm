	...
	if (istype(W, /obj/item/weapon/tank))
		...
		if (istype(W, /obj/item/weapon/screwdriver))
			...
			if (src.c_status)
				src.anchored = 0
				src.c_status = 0
				user.show_message("<span class='info'>You have disconnected the siphon.</span>")
				if(con)
					con.connected = null
			else
				if (con && !con.connected)
					src.anchored = 1
					src.c_status = 3
					user.show_message("<span class='info'>You have connected the siphon.</span>")
					con.connected = src
				else
					user.show_message("<span class='info'>There is nothing here to connect to the siphon.</span>")
			...
			if (istype(W, /obj/item/weapon/wrench))
				src.alterable = !( src.alterable )
				if (src.alterable)
					user << "<span class='info'>You unlock the interface!</span>"
				else
					user << "<span class='info'>You lock the interface!</span>"
	return

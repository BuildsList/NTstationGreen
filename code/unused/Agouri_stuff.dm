	...
	if (istype(W, /obj/item/weapon/weldingtool) && W:welding)
			...
			if (W && istype(src, /turf/simulated/wall))
				if ((get_turf(user) == T && user.equipped() == W))
					...
					for(var/mob/O in viewers(user, 5))
						O.show_message(text("<span class='info'>The wall was sliced apart by []!</span>", user), 1, text("<span class='warning'>You hear metal being sliced apart.</span>"), 2)
		return
		...
		if (W && istype(src, /turf/simulated/wall))
			if ((user.loc == T && user.equipped() == W))
				dismantle_wall(1)
				for(var/mob/O in viewers(user, 5))
					O.show_message(text("<span class='info'>The wall was drilled apart by []!</span>", user), 1, text("<span class='warning'>You hear metal being drilled appart.</span>"), 2)
		return
		...
		if (W && istype(src, /turf/simulated/wall))
			if ((user.loc == T && user.equipped() == W))
				...
				dismantle_wall(1)
				for(var/mob/O in viewers(user, 5))
					O.show_message(text("<span class='info'>The wall was sliced apart by []!</span>", user), 1, text("<span class='warning'>You hear metal being sliced and sparks flying.</span>"), 2)
		return

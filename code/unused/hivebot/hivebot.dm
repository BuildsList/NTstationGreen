/mob/living/silicon/hivebot/New(loc,mainframe)
	src << "<span class='info'>Your icons have been generated!</span>"
	updateicon()
...
/mob/living/silicon/hivebot/meteorhit(obj/O as obj)
	for(var/mob/M in viewers(src, null))
		M.show_message(text("<span class='info'>[src] has been hit by [O]</span>"), 1)
		//Foreach goto(19)
	if (src.health > 0)
		...
		if(ismob(AM))
			...
			if(istype(tmob, /mob/living/carbon/human) && tmob.mutations & FAT)
				if(prob(20))
					for(var/mob/M in viewers(src, null))
						if(M.client)
							M << M << "<span class='info'><B>[src] fails to push [tmob]'s fat ass out of the way.</B></span>"
					src.now_pushing = 0
	...
	if (istype(W, /obj/item/weapon/weldingtool) && W:welding)
		if (W:remove_fuel(0))
			...
			for(var/mob/O in viewers(user, null))
				O.show_message(text("<span class='info'>[user] has fixed some of the dents on [src]!</span>"), 1)
		else
		...
		*/
		...
		if(!(src.lying))
			...
			if (randn <= 40)
				...
				for(var/mob/O in viewers(src, null))
					O.show_message(text("<span class='info'><B>[] has pushed back []!</B></span>", M, src), 1)
			else
				playsound(src.loc, 'sound/weapons/slashmiss.ogg', 25, 1, -1)
				for(var/mob/O in viewers(src, null))
					O.show_message(text("<span class='info'><B>[] attempted to push back []!</B></span>", M, src), 1)
	return
						...
						if (prob(75))
							...
							if (istype(G, /obj/item/weapon/grab))
								for(var/mob/O in viewers(M, null))
									O.show_message(text("<span class='info'>[G.affecting] has been pulled from [G.assailant]'s grip by [src]</span>"), 1)
								del(G)
						else
							ok = 0
...
/mob/living/silicon/hivebot/proc/return_mainframe()
	if(mainframe)
		...
		src << "<span class='info'>You lack a dedicated mainframe!</span>"
		return
...
/proc/specops_process()
	...
	if (!specops_can_move())
		usr << "<span class='info'>The Special Operations shuttle is unable to leave.</span>"
		return
	...
	for(var/turf/T in get_area_turfs(end_location) )
		...
		M << "<span class='info'>You have arrived to [station_name]. Commence operation!</span>"
...
/obj/machinery/computer/specops_shuttle/attackby(I as obj, user as mob)
	if(istype(I,/obj/item/weapon/card/emag))
		user << "<span class='info'>The electronic systems in this console are far too advanced for your primitive hacking peripherals.</span>"
	else
		return attack_hand(user)
...
/obj/machinery/computer/specops_shuttle/attack_hand(var/mob/user as mob)
	...
	if (sent_strike_team == 0)
		usr << "<span class='info'>The strike team has not yet deployed.</span>"
		return
...
/obj/machinery/computer/specops_shuttle/Topic(href, href_list)
	...
	if (href_list["sendtodock"])
		...
		usr << "<span class='info'>Central Command will not allow the Special Operations shuttle to return.</span>"
		return
		...
		usr << "<span class='info'>The Special Operations shuttle will arrive on [station_name] in [(SPECOPS_MOVETIME/10)] seconds.</span>"

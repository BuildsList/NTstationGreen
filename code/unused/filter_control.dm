...
/obj/machinery/filter_control/attackby(obj/item/weapon/W, mob/user as mob)
	...
	if(istype(W, /obj/item/weapon/screwdriver))
		...
		user.show_message(text("<span class='info'>Now [] the panel...</span>", (src.locked) ? "unscrewing" : "reattaching"), 1)
		sleep(30)
	...
	if(istype(W, /obj/item/weapon/wirecutters) && !src.locked)
		...
		for(var/mob/O in viewers(user, null))
			O.show_message(text("<span class='info'>[] has []activated []!</span>", user, (stat&BROKEN) ? "de" : "re", src), 1)
		src.updateicon()
		return
	if(istype(W, /obj/item/weapon/card/emag) && !emagged)
		emagged++
		for(var/mob/O in viewers(user, null))
			O.show_message(text("<span class='info'>[] has shorted out the []'s access system with an electromagnetic card!</span>", user, src), 1)
		src.updateicon()
		return src.attack_hand(user)
	return src.attack_hand(user)

...
/datum/tension
	...
	Topic(href, href_list)
		...
		if(!usr.client.holder)
			message_admins("<span class='info'>[key_name(usr)] tried to use the tensioner without authorization.</span>")
...
//			for(var/i = 0, i < 4, i++) // Four rune words
	...
	/*
		if (emergency_shuttle.direction == 1 && emergency_shuttle.online == 1)
			...
			world << "<span class='info'><B>Alert: The shuttle is going back!</B></span>"
		...
		if(candidates.len)
			...
			for (var/obj/effect/landmark/L in /area/syndicate_mothership/elite_squad)
				...
				if (L.name == "Syndicate-Commando")
					...
					new_syndicate_commando << "<span class='info'>You are an Elite Syndicate. [!syndicate_leader_selected?"commando":"<B>LEADER</B>"] in the service of the Syndicate. \nYour current mission is: \red<B> [input]</B></span>"

...
/obj/machinery/dna_scanner/verb/move_inside()
	...
	if (usr.stat != 0)
		return
	if (src.occupant)
		usr << "<span class='info'><B>The scanner is already occupied!</B></span>"
		return
	if (usr.abiotic())
...
/obj/machinery/dna_scanner/attackby(obj/item/weapon/grab/G as obj, user as mob)
	if ((!( istype(G, /obj/item/weapon/grab) ) || !( ismob(G.affecting) )))
		return
	if (src.occupant)
		user << "<span class='info'><B>The scanner is already occupied!</B></span>"
		return
	if (G.affecting.abiotic())
		user << "<span class='info'><B>Subject cannot have abiotic items on.</B></span>"
		return
...
/obj/machinery/restruct/verb/operate()
	...
	if ((src.occupant && src.occupant.primary))
		...
		if (istype(src.occupant, /mob/living/carbon/human))
			...
			H << text("<span class='info'><B>Your name is now [].</B></span>", H.real_name)
	return
...
/obj/machinery/restruct/attackby(obj/item/weapon/grab/G as obj, user as mob)
	...
	if ((!( istype(G, /obj/item/weapon/grab) ) || !( ismob(G.affecting) )))
		return
	if (src.occupant)
		user << "<span class='info'><B>The machine is already occupied!</B></span>"
		return
	if (G.affecting.abiotic())
		user << "<span class='info'><B>Subject cannot have abiotic items on.</B></span>"
		return

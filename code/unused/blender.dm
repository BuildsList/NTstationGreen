...
/obj/machinery/blender/verb/blend()		//Blend shit. Note: In the actual blending loop, make sure it can't include the jug.
	...
	if (src.stat != 0) //NOPOWER etc
		return
	if(src.processing)
		usr << "<span class='info'>The blender is in the process of blending.</span>"
		return
	if(!src.container)
		usr << "<span class='info'>The blender doesn't have an attached container!</span>"
		return
	playsound(src.loc, 'sound/machines/blender.ogg', 50, 1)
	src.processing = 1
	usr << "<span class='info'>You turn on the blender.</span>"
	use_power(250)
